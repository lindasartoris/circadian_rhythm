...
for (i in 1:100){#####perform 100 randomisations
  ...
  for (interac in interac_list){
    ...
    root_name          <- unlist(strsplit(interac,split="/"))[grepl("interactions",unlist(strsplit(interac,split="/")))] # LS: we had to change grepl("colony",...) to "interactions" because I don't have the word "colony" in my files
